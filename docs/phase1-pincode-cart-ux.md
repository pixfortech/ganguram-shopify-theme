...
- **Out‑of‑stock = the client‑side pickup widget.** “Out of stock at Beadon Street” is the theme’s `pickup-availability-compact` (not `product.available`), so CSS on `.product-item--sold-out` can’t catch it. After it resolves, the gate flags the card `data-ganguram-oos-hidden` when the widget shows the unavailable alert (`.alert--note`, not `.alert--success`) and the CSS hides it. Globally sold‑out products are additionally filtered in Liquid (`collection-panel.liquid`, the Best‑Sellers loop) and hidden via `.product-item--sold-out`.
...
```js
GanguramStorefrontGate.debugState();   // phase1CssLoaded, hasValidPincode, gateActive, gatedButtons, ctaButtons, pickupWidgets, outOfStockHidden, gatedSelectors
