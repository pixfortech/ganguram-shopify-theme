...
- **Risk:** **medium** — (1) the 2026 `agents.md.liquid` capability is **WebSearch-verified but postdates this assistant's training — confirm on shopify.dev first**; (2) a custom template **replaces** (does not merge with) Shopify's auto-generated default, so it could **drop the auto UCP/agent-discovery links** — and the live default (now verified 2026-06-27) includes UCP discovery (`/.well-known/ucp`), the MCP endpoint (`/api/ucp/mcp`), the Shop skill and the **human-approval-before-payment** rule, all of which the custom file must **preserve**; (3) text endpoints only — **zero storefront UI impact**.
