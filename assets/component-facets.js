  ...
  static renderSectionFromFetch(url, event) {
    fetch(url)
      ...
      .then((responseText) => {
        ...
        var expectedValue = document.querySelector('.header-info-block').getAttribute('data-code-list');
      var expectedCodesArray = expectedValue.split(',');
      var get_url = window.location.href;       
      if (document.body.classList.contains('template-collection')) {        
         const currentUrl = new URL(window.location.href);
         // Append the desired strings
         var modifiedUrl = `/collections/all/products.json${currentUrl.search}&limit=500`;
      }else{
          var modifiedUrl = '/collections/all/products.json?limit=500';   
      }
      console.log(modifiedUrl,'modifiedUrl');
      const apiEndpoint = modifiedUrl;    
      fetch(apiEndpoint)
        .then(response => response.json())
        .then(data => {
          console.log('innnnooooopppp');
          data.products.forEach(product => {
            if (expectedCodesArray.includes(storedValue)) {                               
              if(product.tags == 'Kolkata'){
                console.log('Product Title:', product.tags); 
                
                  var elements = document.querySelectorAll('.product-item.' + product.tags);
                  ...
                  if (elements.length === 0 && product.tags){                    
                    var singleElement = document.querySelector('.product-item.' + product.tags);
                    if (singleElement) {
                      console.log('innnnooooopppp');
                  ...
                  localStorage.setItem("Zipcode", storedValue);
              }
            }else{
             ...
             console.log(get_class,'get_classget_class');
               var pincodeElement = document.querySelectorAll('.pincode');
                for(var i=0;i<=pincodeElement.length;i++){
                  ...
                  pincodeElement[i].innerHTML += storedValue;                    
                }                       
              var CollectionProductCount =  document.querySelector('#CollectionProductCount').innerHTML == '';
